...
$(function() {
  ...
  //マウスオーバーでimg切り替え
  $("body").on("mouseenter", ".mouseHoverImg", function(){
    $(this).find("img").attr("src", $(this).find("img").attr("src").replace("_off", "_on"));
  });
  $("body").on("mouseleave", ".mouseHoverImg", function(){
    $(this).find("img").attr("src", $(this).find("img").attr("src").replace("_on", "_off"));
  });


  $("body").on("click", ".tbl__link", function(){
    $(this).next(".tblPopup").show();
    return false;
  });
  $("body").on("click", ".tblPopup__close", function(){
    $(this).parent(".tblPopup__nav").parent(".tblPopup").hide();
    return false;
  });
  ...
  $("body").on("click", ".openSwitch", function(){
    ...
    if($(".openBox--" + target).css("display") == "block"){
      ...
      if(dataType == 1){
        self.html('<img src="assets/img/icon_down__off.png" alt=""> <span>もっと詳細な条件</span>');
      } else if(dataType == 2){
      ...
      if(dataType == 1){
        self.html('<img src="assets/img/icon_up__off.png" alt=""> <span>閉じる</span>');
      } else if(dataType == 2){
        self.html("&#8743; <span>グレード絞り込み</span>");
  ...
  $("body").on("click", ".modal", function(){
    $(".modalArea").removeClass("imgModal");
    if($(this).hasClass("imgModalBtn")){
      $(".modalArea").addClass("imgModal");
    }
